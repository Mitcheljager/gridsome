<script lang="ts">
  ...
  interface Props { levelId: string }

  const { levelId } : Props = $props()

  let completedLevels: CompletedLevel[] = $state([])

  const stars: number[] = $derived(levels.find(l => l.id === levelId)!.stars)
  const bestScore: number = $derived(completedLevels.find(l => l.id === levelId)?.score || 0)
...
<div class="stars">
  {#each stars as star, i}
    ...
    <div class="star" class:active in:scale|global={{ duration: 1000, delay: 100 * i, start: 0.75, easing: elasticOut }}>
